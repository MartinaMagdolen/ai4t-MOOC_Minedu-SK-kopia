---
názov: 0.2.1 Podmienky používania obsahu MOOC kurzu
popis: Podmienky používania zdrojov z MOOC kurzu - licencie a bezpečnostné opatrenia
---
??? info "Metadáta
    - Id: EU.AI4T.O1.M0.2.1t
    - Názov: 0.2.1 Podmienky používania obsahu MOOC kurzu
    - Typ: text
    - Opis: Podmienky používania zdrojov z MOOC kurzu - licencie a bezpečnostné opatrenia
    - Predmet: Umelá inteligencia pre učiteľov a od učiteľov
    - Autori:
        - AI4T
    - Licencia: CC BY 4.0
    - Dátum: 2022-11-15

# Podmienky používania obsahu MOOC kurzu

## Podmienky používania

Celý obsah tohto MOOC kurzu, ktorý vyvinuli partneri konzorcia AI4T, môžete využívať podľa svojich cieľov a potrieb.  
Konzorcium AI4T sa rozhodlo uplatniť licenciu, ktorá umožňuje rôzne používanie a prispôsobovanie obsahu MOOC kurzu.

### Používanie vzdelávacích zdrojov z MOOC kurzu

Pokiaľ nie je uvedené inak, **vzdelávacie zdroje** sa šíria pod licenciou [Creative Commons CC-BY 4.0] (https://creativecommons.org/licenses/by/4.0/deed.fr):

**Uvedenie autora**. Držiteľ práv povoľuje akékoľvek použitie diela vrátane použitia na komerčné účely, ako aj vytváranie odvodených diel, ktorých šírenie je tiež povolené bez obmedzenia. To všetko je možné za predpokladu, že sa pri takomto použití uvedie meno pôvodného autora.

* Ako uviesť autora pri použití MOOC kurzu ako školenia:
  MOOC kurz „Umelá inteligencia pre učiteľov a od učiteľov“, AI4T, publikované pod [licenciou CC-BY](https://creativecommons.org/licenses/by/4.0/deed.fr). Nezabudnite pridať logo programu Erasmus+:
<figure>
  <img src="Images/LogoCoFoundedErasmusProgramEU.png" alt="Logo Co-founded by Erasmus and EU"/>
</figure>

* Ako uviesť autora pri použití MOOC kurzu ako zdroja:
  Zdroj: AI4T (Autor) „Názov zdroja“, prevzaté z „Umelá inteligencia pre učiteľov a od učiteľov“, AI4T, publikované pod [licenciou CC-BY](https://creativecommons.org/licenses/by/4.0/deed.en). Nezabudnite pridať logo programu Erasmus+:
<figure>
  <img src="Images/LogoCoFoundedErasmusProgramEU.png" alt="Logo Co-founded by Erasmus and EU"/>
</figure>
